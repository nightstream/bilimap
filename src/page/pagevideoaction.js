...
Element.prototype.attachShadow = function () {
    ...
    let result = this._attachShadow( { mode: "open" } );
...
function getVideoObj(){
    ...
    let div = document.getElementById("bilibiliPlayer") || document.getElementById("bilibili-player");
    if (!div)
        return;
    let video = div.getElementsByTagName("video");
    if (video.length == 0){
        // let bwp = div.getElementsByTagName("bwp-video");
...
function go2origin(e){
    console.log("事件触发");
    let obj = getVideoObj() || e.target;
    let uuid = obj.getAttribute("data-loopuid");
    if (uuid == undefined)
        return;
...
function regKeyEvent(event){
    ...
    if (!isin(keycode, ["KeyZ", "KeyX"]))
        return;
    let video_obj = getVideoObj();
    // 如果没获取到播放器
    if (! video_obj)
        return

    let uuid = video_obj.getAttribute("data-loopuid");
    if (keycode == "KeyX"){
        // clear
